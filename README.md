...
### 回测结果（2022-01 ~ 2026-05，22支原版股票池）

> 回测条件：初始资金 100 万，A 股实盘费率（佣金万 2.5 + 印花税万 5 卖出），含熔断恢复机制。

| 配置 | 总收益 | 年化收益 | 最大回撤 | 夏普比率 | 说明 |
|------|--------|----------|---------|----------|------|
| **max5 · 等权重** | **+132.92%** | **+21.65%** | **-15.50%** | **1.07** | 推荐配置 |
| max5 · 信号加权 | +132.97% | +21.66% | -15.56% | 1.07 | 与等权几乎一致，同质化强 |
| max5 · 波动率平价 | +128.57% | +21.12% | -15.41% | 1.06 | 效果接近，额外复杂度无收益 |
| max3 · 等权重 | +91.88% | +16.31% | -14.95% | 0.92 | 仓位过少，错过同期上涨机会 |

**关键发现：**

- 三种仓位分配方式（等权 / 信号加权 / 波动率平价）结果高度趋同。在 22 支同属 AI/半导体板块的股票池中，信号强度差异不足以产生有意义的权重分化，**等权重是最简单有效的选择**。
- 此前录得的 +62.57% 源于一个 bug：止损熔断后引擎永久退出，不再恢复交易。修复熔断恢复逻辑后，市场回暖时引擎能自动重新入场，收益从 +62% 跃升至 **+132%**。这说明在 A 股强趋势市场中，**"能回来"比"跑得快"更重要**。
- 尝试将股票池从 22 支扩大到 200 支后，收益反而转负。策略的 alpha 高度集中于 AI/半导体等强趋势板块；宇通客车、中国中免等非科技票在策略中极少触发信号，客观上充当了天然的低仓位缓冲。
